...
class ShowProfessorBySpecialiteViewModel : ViewModel() {

    val professorSpecList: MutableLiveData<List<Professor>> by lazy {
    ...
    fun getProfessors(specId:Int) {
//        Create a call to get the specialite
        val call: Call<List<Professor>> = RetrofitClient.getInstance().springAPI.getProfessorBySpecialite(specId)
...
//        Queue the call to add the logic to what's gonna be done with the data
        call.enqueue(object : Callback<List<Professor>> {
            override fun onResponse(
                ...
                professorSpecList.value = response.body() ?: emptyList()
                Log.d(
                    "Call Success",
