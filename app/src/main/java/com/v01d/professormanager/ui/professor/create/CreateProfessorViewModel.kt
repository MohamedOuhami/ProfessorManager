...
class CreateProfessorViewModel : ViewModel() {
    ...
    fun createProfessor(first_name:String, last_name:String, dateNaissance: Date?, specialite: Specialite){
        val professor:Professor = Professor(0,first_name,last_name, null, specialite)

        val call: Call<Void> = RetrofitClient.getInstance().springAPI.createProfessor(professor)
